...
exports.signIn = async (req, res) => {
    ...
    try {
        ...
        const uploadResult = await imageUploader(profile_picture);
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (isMatch) {
            ...
            const token = jwt.sign(payload, "hostheven", { expiresIn: "30h" })
            userExist.token = token;
            res.cookie("token", token, { expires: new Date(Date.now() + 30 * 34 * 60 * 1000) }).status(200).json({ success: true, message: "Login successfully", user: userExist })
        } else {
